...
export default function Chat() {
    ...
    onError: (error) => {
      ...
      if (isRateLimit) {
        toast.error("Rate limit exceeded", {
          description: "You've hit the API rate limit. Please wait a moment and try again.",
          richColors: true,
          position: "top-center",
          duration: 5000,
        });
  ...
  const refreshBrowser = async () => {
    try {
      ...
      const { streamUrl, id } = await getBrowserURL(sandboxId || undefined);
      setStreamUrl(streamUrl);
      setSandboxId(id);
    } catch (err) {
      console.error("Failed to refresh browser:", err);
    } finally {
      setIsInitializing(false);
    }
  };

  // Kill browser on page close
  useEffect(() => {
    if (!sandboxId) return;

    // Function to kill the browser - just one method to reduce duplicates
    const killBrowser = () => {
      if (!sandboxId) return;
    ...
    if (isIOS || isSafari) {
      ...
      window.addEventListener("pagehide", killBrowser);

      return () => {
        window.removeEventListener("pagehide", killBrowser);
        // Also kill browser when component unmounts
        killBrowser();
      };
    } else {
      // For all other browsers, use beforeunload
      window.addEventListener("beforeunload", killBrowser);

      return () => {
        window.removeEventListener("beforeunload", killBrowser);
        // Also kill browser when component unmounts
        killBrowser();
      };
    }
  ...
  useEffect(() => {
    // Initialize browser and get stream URL when the component mounts
    const init = async () => {
      try {
        setIsInitializing(true);
          ...
          >
            {streamUrl ? (
              <>
                ...
                >
                  {isInitializing ? "Creating browser..." : "New browser"}
                </Button>
              </>
            ) : (
              <div className="flex items-center justify-center h-full text-white">
                {isInitializing ? "Initializing browser..." : "Loading stream..."}
              </div>
            )}
          </ResizablePanel>
